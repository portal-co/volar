...
impl<T: Clone> Clone for DeltaDyn<T> {
    fn clone(&self) -> Self {
        DeltaDyn {
            delta: self.delta.iter().cloned().collect(),
        }
    }
}
...
impl<T: PartialEq> PartialEq for DeltaDyn<T> {
    fn eq(&self, other: &Self) -> bool {
        if self.delta.len() != other.delta.len() {
            return false;
        }
        for i in 0..self.delta.len() {
            if self.delta[i] != other.delta[i] {
                return false;
            }
        }
        true

...
use super::*;
impl<N: VoleArray<BitsInBytes>, K: ArrayLength<GenericArray<BitsInBytes, N>>>
    Vope<N, BitsInBytes, K>
{
    pub fn rotate_left_bits(&self, n: usize) -> Self {
        let Vope { u, v } = self;
        Vope {
            u: GenericArray::<GenericArray<BitsInBytes, N>, K>::generate(|l| {
                GenericArray::<BitsInBytes, N>::generate(|i| {
                    let BitsInBytes(b) = u[l][i].clone();
                    let BitsInBytes(next) = u[l][(i + 1) % N::to_usize()].clone();
                    BitsInBytes(b.shl(n as u32) | next.shr(8 - n as u32))
                })
            }),
            v: GenericArray::<BitsInBytes, N>::generate(|i| {
        ...
        K: ArrayLength<GenericArray<Bit, N>>,
    {
        let Vope { u, v } = self;
        Vope {
            u: GenericArray::<GenericArray<Bit, N>, K>::generate(|l| {
                GenericArray::<Bit, N>::generate(|i| {
                    let BitsInBytes(b) = u[l][i].clone();
                    Bit((b >> n) & 1 != 0)
                })
            }),
            v: GenericArray::<Bit, N>::generate(|i| {
                let BitsInBytes(b) = v[i].clone();
                Bit((b >> n) & 1 != 0)
            }),
        }
...
impl<N: VoleArray<BitsInBytes64>, K: ArrayLength<GenericArray<BitsInBytes64, N>>>
    Vope<N, BitsInBytes64, K>
{
    pub fn rotate_left_bits(&self, n: usize) -> Self {
        let Vope { u, v } = self;
        Vope {
            u: GenericArray::<GenericArray<BitsInBytes64, N>, K>::generate(|l| {
                GenericArray::<BitsInBytes64, N>::generate(|i| {
            ...
            v: GenericArray::<BitsInBytes64, N>::generate(|i| {

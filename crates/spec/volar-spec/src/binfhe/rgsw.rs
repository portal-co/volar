...
>(
    ...
    let rows = core::array::from_fn(|j| {
        ...
        // a-column: encrypt zero, torus_add m * g_j to a[0] so the phase carries
        // -m * g_j * s(X).
...
/// RGSW noise.
pub fn binfhe_external_product<
    const N: usize,
    const LOG: u32,
    const ELL: usize,
    const BASE_LOG: u32,
...
mod tests {
    ...
    use crate::binfhe::rlwe::{binfhe_rlwe_phase, binfhe_rlwe_trivial};
    ...
    fn external_product_scales_phase_by_the_encrypted_bit() {
        ...
        let mut content = binfhe_rlwe_trivial::<{ toy::BIG_N }, 8>(&[0u32; toy::BIG_N]);
    ...
    fn cmux_selects_the_correct_operand() {
        ...
        let mut d1 = binfhe_rlwe_trivial::<{ toy::BIG_N }, 8>(&[0u32; toy::BIG_N]);
        for i in 0..toy::BIG_N {

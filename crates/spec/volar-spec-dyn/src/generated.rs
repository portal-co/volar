...
pub fn double_vec<B: LengthDoubler>(v: Vec<u8>) -> [Vec<u8>; 2] {
    let ga = generic_array::GenericArray::from_exact_iter(v.into_iter())
        .expect("double_vec: length mismatch");
    let [a, b] = B::double(ga);
    [a.to_vec(), b.to_vec()]
}
...
impl<T> DeltaDyn<T> {
    pub fn remap<F: FnMut(usize) -> usize>(&self, mut m: usize, mut f: F) -> DeltaDyn<T>
    where
        T: Clone,
    {
        let n: usize = self.n;
    ...
    pub fn r#static<U: Mul<T, Output = O> + Clone, O>(&self, mut val: Vec<U>) -> QDyn<O>
    where
        T: Clone,
    {
        let n: usize = self.n;
        QDyn {
            q: (0..n)
                .map(|i| (val[i].clone() * self.delta[i].clone()))
                .collect::<Vec<O>>(),
            n: 0,
        }
    }
...
impl<T> QDyn<T> {
    pub fn remap<F: FnMut(usize) -> usize>(&self, mut m: usize, mut f: F) -> QDyn<T>
    where
        T: Clone,
    {
        let n: usize = self.n;
        let Self { q: q, .. } = self;
        QDyn {
            q: (0..m).map(|i| q[(f(i) % n)].clone()).collect::<Vec<T>>(),
            n: 0,
        }
    }
    pub fn rotate_left(&self, mut n: usize) -> Self
...
impl<T> PolyDyn<T> {
    pub fn get_qs_pool<Q: Clone + Mul<A, Output = A>, A: Add<A, Output = A>>(
        &self,
        mut m: usize,
        mut x: usize,
        mut root: DeltaDyn<Q>,
        mut inputs: PolyInputPoolDyn<QDyn<Q>>,
        mut reduction: usize,
    ) -> QDyn<A>
    where
        T: Clone + Into<A>,
    ...
    pub fn get_qs<Q: Clone + Mul<A, Output = A>, A: Add<A, Output = A>>(
        &self,
        mut m: usize,
        mut x: usize,
        mut root: DeltaDyn<Q>,
        mut inputs: Vec<Vec<QDyn<Q>>>,
        mut reduction: usize,
    ) -> QDyn<A>
    where
        T: Clone + Into<A>,
    ...
    pub fn apply_pool<M, O: Mul<O, Output = O> + Add<O, Output = O> + Default + Clone>(
        &self,
        mut m: usize,
        mut x: usize,
        mut x2: usize,
        mut xs: usize,
        mut s: usize,
        mut voles: &PolyInputPoolDyn<VopeDyn<T>>,
    ) -> VopeDyn<O>
    where
        T: Into<O> + Clone,
    {
...
impl<T: Clone> Clone for VopeDyn<T> {
    fn clone(&self) -> Self {
        ...
        VopeDyn {
            u: (0..k)
                .map(|l| (0..n).map(|i| u[l][i].clone()).collect::<Vec<T>>())
                .collect::<Vec<Vec<T>>>(),
            v: (0..n).map(|i| v[i].clone()).collect::<Vec<T>>(),
            n: 0,
            k: 1,
        }
    }
...
impl<T: Clone> Clone for QDyn<T> {
    fn clone(&self) -> Self {
        ...
        let QDyn { q: q, .. } = self;
        QDyn {
            q: (0..n).map(|i| q[i].clone()).collect::<Vec<T>>(),
            n: 0,
        }
    }
}
...
impl<T: Clone> Clone for DeltaDyn<T> {
    fn clone(&self) -> Self {
        ...
        DeltaDyn {
            delta: (0..n).map(|i| delta[i].clone()).collect::<Vec<T>>(),
            n: 0,
        }
    }
}
...
impl<T> VopeDyn<T> {
    pub fn constant(mut n: usize, mut v: Vec<T>) -> Self {
        let k: usize = 0;
        VopeDyn {
            u: (0..0).map(|_| unreachable!()).collect::<Vec<Vec<T>>>(),
            v: v,
            n: 0,
            k: 1,
        }
...
impl<T: BitXor<U, Output = O> + Clone + Into<O>, U: Clone, O> BitXor<Vec<U>> for VopeDyn<T>
where
    T: Into<O>,
{
    type Output = VopeDyn<O>;
    fn bitxor(self, mut rhs: Vec<U>) -> Self::Output {
        ...
        VopeDyn {
            u: (0..k)
                ...
                .collect::<Vec<Vec<O>>>(),
            v: self.v.into_iter().map(|a| a.into()).collect::<Vec<_>>(),
            n: 0,
            k: 1,
        }
    }
...
impl<T: Mul<U, Output = O> + Into<O> + Clone, U: Mul<U, Output = U> + Clone, O: Add<O, Output = O>>
    Mul<DeltaDyn<U>> for VopeDyn<T>
{
    type Output = QDyn<O>;
...
impl<T> VopeDyn<T> {
        ...
        T: Clone + Default,
    {
        ...
        VopeDyn {
            u: (0..l)
                .map(|l| {
                    (0..n)
                        .map(|i| u.get(l).map_or(T::default(), |a| a[i].clone()))
                        .collect::<Vec<T>>()
                })
                .collect::<Vec<Vec<T>>>(),
            v: v.clone(),
    ...
    pub fn remap<F: FnMut(usize) -> usize>(&self, mut m: usize, mut f: F) -> VopeDyn<T>
    where
        T: Clone,
    {
        let n: usize = self.n;
        ...
        VopeDyn {
            u: (0..k)
                .map(|l| (0..m).map(|i| u[l][(f(i) % n)].clone()).collect::<Vec<T>>())
                .collect::<Vec<Vec<T>>>(),
            v: (0..m).map(|i| v[(f(i) % n)].clone()).collect::<Vec<T>>(),
            n: 0,
            k: 1,
        }
    }
                    ...
                    (0..u)
                        .map(|j| {
                            ...
                            if bad.contains(&(i2 as u64)) {
                                ...
                                (0..m)
                                    .map(|j| self.per_byte[i2].get(j).cloned().unwrap_or_default())
                                    .collect::<Vec<u8>>()
                            }
                        })
                        .collect::<Vec<Vec<u8>>>()
...
impl<B: LengthDoubler, D: Digest> ABOOpeningDyn<B, D> {
    ...
    pub fn to_vole_material_expanded<X: AsRef<[u8]>, F: FnMut(&[u8]) -> X>(
        &self,
        mut n: usize,
        mut f: F,
    ) -> Vec<VopeDyn<u8>> {
    ...
    pub fn to_vole_material_typenum_expanded<X: AsRef<[u8]>, F: FnMut(&[u8]) -> X>(
        &self,
        mut n: usize,
        mut f: F,
    ) -> Vec<VopeDyn<u8>> {
    ...
    pub fn split_bit_typenum(&self, mut n: usize) -> Vec<BSplitDyn<B, D>>
    where
        D: Digest,
    {
        let t: usize = self.t;
...
pub fn gen_abo<B: LengthDoubler, D: Digest>(
    mut k: usize,
    mut a: Vec<u8>,
    mut rand: &impl AsRef<[u8]>,
) -> ABODyn<B, D>
where
    B: Sized,
{
...
pub fn create_vole_from_material<B: LengthDoubler, X: AsRef<[u8]>>(mut s: &[X]) -> VopeDyn<u8> {
    let u: Vec<u8> = s.iter().fold(
        (0..<<B>::OutputSize as Unsigned>::to_usize())
            .map(|_| <u8>::default())
            .collect::<Vec<u8>>(),
        |mut a, b| {
            a.into_iter()
    ...
    let v: Vec<u8> = s.iter().enumerate().fold(
        (0..<<B>::OutputSize as Unsigned>::to_usize())
            .map(|_| <u8>::default())
            .collect::<Vec<u8>>(),
        |mut a, (i, b)| {
            a.into_iter()
                ...
                .map(|(a, b)| a.bitxor(b).bitxor((i as u8)))
                .collect::<Vec<_>>()
        },
    );
    VopeDyn {
        u: (0..1).map(|_| u.clone()).collect::<Vec<Vec<u8>>>(),
        v: v,
        n: 0,
        k: 1,
    }
...
>(
    ...
    let u: Vec<u8> = s
        .iter()
        .map(|b| f(&AsRef::<[u8]>::as_ref(&b)[..<<B>::OutputSize as Unsigned>::to_usize()]))
        .fold(
            (0..<<B>::OutputSize as Unsigned>::to_usize())
                .map(|_| <u8>::default())
                .collect::<Vec<u8>>(),

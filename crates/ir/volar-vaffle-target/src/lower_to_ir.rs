...
struct BlockEmitter<P: Clone = ()> {
    ...
    current_groups: volar_ir_common::GroupMembership,
    next_var: u32,
}
...
impl<P: Clone> BlockEmitter<P> {
    fn new(params: Vec<IRTypeId>) -> Self {
        ...
        BlockEmitter {
            ...
            stmts: Vec::new(),
            current_prov: None,
            current_groups: volar_ir_common::GroupMembership::empty(),
            next_var,
        }
    }
    fn set_prov(&mut self, prov: P) { self.current_prov = Some(prov); }
    fn set_groups(&mut self, groups: volar_ir_common::GroupMembership) {
    ...
    fn emit(&mut self, stmt: IRStmt) -> IRVarId {
        ...
        self.stmts.push(
            volar_ir_common::Node::new(stmt, prov, None)
...
struct LowerCtx<'m, P: Clone = ()> {
    ...
    instruction_groups: Vec<volar_ir_common::InstructionGroupDecl>,
    instruction_group_instances: Vec<volar_ir_common::InstructionGroupInstance<volar_ir::ir::IRGroupValueRef>>,
    /// Maps `(function, VAFFLE ValueId)` to the IR variable emitted for that
    /// source value, including entry and block parameters.
    value_map: BTreeMap<(usize, usize), (IRBlockId, IRVarId)>,
...
impl<'m, P: Clone> LowerCtx<'m, P> {
    ...
    fn lower_all(&mut self) {
        ...
        self.lower_instruction_group_instances();

        // Append extra blocks (continuations created by call splitting).
        self.blocks.append(&mut self.extra_blocks);
    }

    fn lower_instruction_group_instances(&mut self) {
        for (func_idx, function) in self.module.funcs.iter().enumerate() {
            let FuncDecl::Body(body) = function else { continue };
            for instance in &body.instruction_group_instances {
                let inputs = instance.inputs.iter().map(|input| {
                    let vars = input.values.iter().map(|value| {
                        let (block, var) = self.value_map.get(&(func_idx, value.0)).copied()
                            .expect("instruction-group capture was not lowered to Volar IR");
                        (block, var)
                    }).collect::<Vec<_>>();
                    let block = vars.first().map(|(block, _)| *block)
                        .expect("instruction-group packed input must contain at least one value");
                    assert!(vars.iter().all(|(candidate, _)| *candidate == block),
                        "instruction-group packed input spans IR blocks");
                    volar_ir::ir::IRGroupValueRef {
                        block,
                        vars: vars.into_iter().map(|(_, var)| var).collect(),
                        ty: self.type_map[input.ty.0 as usize],
                    }
                }).collect();
                self.instruction_group_instances.push(volar_ir_common::InstructionGroupInstance {
                    id: instance.id,
                    decl: instance.decl,
                    inputs,
                });
        ...
        for (vaffle_bi, vaffle_block) in body.blocks.iter().enumerate() {
            ...
            if is_entry {
                ...
                for (pi, &bit) in all_bits.iter().enumerate() {
                    if pi < vaffle_block.params.len() {
                        let value = vaffle_block.params[pi].0.0;
                        val_map.insert(value, bit);
                        self.value_map.insert((func_idx, value), (IRBlockId(ir_bi as u32), bit));
                    }
                }
            } else {
                // Non-entry: map block params (after packed SP words).
                for (pi, &(vid, _ty)) in vaffle_block.params.iter().enumerate() {
                    let var = IRVarId((sp_packs + pi) as u32);
                    val_map.insert(vid.0, var);
                    self.value_map.insert((func_idx, vid.0), (IRBlockId(ir_bi as u32), var));
                }
            }
            ...
            while !remaining_stmts.is_empty() {
                ...
                for &svid in before_call.iter() {
                    let source = &body.values[svid.0];
                    current_em.set_prov(source.provenance().clone());
                    current_em.set_groups(source.instruction_groups().clone());
                    match &body.values[svid.0].kind {
                        Value::Op(stmt) => {
                            ...
                            self.value_map.insert((func_idx, svid.0), (IRBlockId(ir_bi as u32), id));
                        }
                        Value::StackAlloc { base_slot, .. } => {
                            ...
                            self.value_map.insert((func_idx, svid.0), (IRBlockId(ir_bi as u32), addr));
                        }
                        Value::PtrLoad { ptr, .. } => {
                            let s = val_map.get(&ptr.0).copied().unwrap_or(IRVarId(0));
                            val_map.insert(svid.0, s);
                            self.value_map.insert((func_idx, svid.0), (IRBlockId(ir_bi as u32), s));
                        }
                        Value::PtrStore { .. } => {}
                        Value::PtrOffset { idx, .. } => {
                ...
                match at_call {
                    Some(call_vid) => {
                        ...
                        current_em.set_groups(body.values[call_vid.0].instruction_groups().clone());
                        if !body.values[call_vid.0].instruction_groups().is_empty() {
                            panic!("lower_vaffle_to_ir: calls inside instruction groups must be inlined or rejected before stack-frame lowering");
                        }

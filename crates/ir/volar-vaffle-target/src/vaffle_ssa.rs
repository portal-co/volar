...
//! clobbering it. To prevent this, every function (other than the
//! module's own designated entry function, which nothing inside the
//! module ever calls) receives an extra, threaded 32-bit "stack pointer"
//! parameter, and every call site advances it by a fixed, generous
//! per-call step (`SP_STEP`, chosen larger than the largest `ValueId` in
//! the whole module) before passing it to the callee. Every spill/reload
//! address becomes `SP + ValueId` (a real bit-level addition, `bc_add`)
//! instead of the raw `ValueId` alone — so nested/recursive invocations,
//! each carrying a different SP, never share an address, regardless of
//! recursion depth. The entry function's own SP is simply hardcoded to
//! zero at its own block 0, since nothing ever calls it recursively (or
//! at all, from within this module).
//!
//! SP itself is threaded through *every* block (not just blocks that
//! spill or use something) as an ordinary block param/jump-arg — the
//! "max SSA" mechanism an earlier version of this pass used for *every*
//! cross-block value, before that was found to scale badly (`Σ over
//! values (blocks on the path from definition to use)` blew up to tens
//! of GB on the real ~120-block, ~44,000-cross-block-value RISC-V
//! interpreter this compiles). Threading is fine for SP specifically
//! because there's only ever one SP (32 bits) per function, not thousands
//! of independent values — the per-block param cost is negligible
//! (`O(n_blocks)`, not `O(n_values × path_length)`).
//!
...
pub fn ssa_ify_function<P: Clone>(
    ...
    // ---- Phase 1: thread a 32-bit SP through every block -------------------
    let sp_bits_for = thread_sp(&mut blocks, &mut values, &rpo, &preds, entry, is_entry, bit_tid);

    // ---- Phase 2: advance SP at every internal call site --------------------
    wire_call_sites(&mut blocks, &mut values, &sp_bits_for, sp_step, bit_tid);
    ...
    let mut uses_per_block: Vec<BTreeSet<usize>> = Vec::with_capacity(blocks.len());
    ...
    for bi in 0..blocks.len() {
        ...
        let sp_bits = sp_bits_for[&bi].clone();
        for &v in uses {
            let vid = ValueId(v);
            let ty = vaffle_value_vtid(module, &values, vid);
            let prov = values[v].provenance().clone();
...
/// Emit `SP + vid` (a real `SPILL_ADDR_BITS`-wide `bc_add`) as a sequence
/// of new value-arena entries, appended directly to `values` (the
/// `vaffle_ssa`-in-progress arena, so this composes with the rest of the
/// pass's own append-only insertion style). Returns the *new* statement
...
fn emit_spill_address<P: Clone>(
    ...
    let vid_bits: Vec<ValueId> = (0..SPILL_ADDR_BITS)
        .map(|i| builder.bc_const(((vid >> i) & 1) != 0))
        .collect();
    let sum_bits = bc_add(&mut builder, sp_bits, &vid_bits, false);
...
/// Thread a fresh 32-bit SP through every block of the function, in RPO
/// order (guaranteeing every non-back-edge predecessor is processed
/// before its successors). Returns each block's own SP bits. The entry
/// function's own SP is a hardcoded zero constant; every other
/// function's own SP is an extra parameter appended to its entry block's
/// existing params (received from its own callers — see
/// [`wire_call_sites`]).
    ...
    for &bi in rpo {
        let bits: Vec<ValueId> = if bi == entry {
            if is_entry_function {
                ...
                let np = blocks[bi].params.len();
                (0..SPILL_ADDR_BITS)
                    .map(|i| {
                        let vid = ValueId(np + i);
                        blocks[bi].params.push((vid, bit_tid));
                        vid
                    })
                    .collect()
            }
...
fn wire_call_sites<P: Clone>(
    ...
    for bi in 0..blocks.len() {
        let sp = sp_bits_for[&bi].clone();
        let stmt_ids: Vec<ValueId> = blocks[bi].stmts.clone();
        for svid in stmt_ids {
...
fn advance_sp<P: Clone>(
    ...
    let step_bits: Vec<ValueId> = (0..SPILL_ADDR_BITS)
        ...
        .collect();
    bc_add(&mut builder, sp, &step_bits, false)
}
...
mod tests {
    ...
    #[test]
    fn test_no_cross_block_values_entry_sp_is_hardcoded_zero() {
        ...
        // block0 (entry, is_entry=true) gains 32 fresh Const(false) stmts
        // for its own hardcoded-zero SP, no params added.
        assert_eq!(out.blocks[0].params.len(), 0, "entry function's own SP is not a param -- hardcoded zero");
        assert_eq!(out.blocks[0].stmts.len(), 1 + SPILL_ADDR_BITS, "original stmt + 32 SP-zero consts");
        // block1 gets 32 SP params (threaded via the Jump edge) but
        // no spill/reload (v0/v1 both used only in their own block).
        assert_eq!(out.blocks[1].params.len(), SPILL_ADDR_BITS);

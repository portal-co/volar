...
use crate::generators::ir::{
    interpret_ir, interpret_ir_extended, interpret_ir_multiblock,
    interpret_ir_with_advisory_group, PRIM_TYPES,
};
...
/// Like [`ArbitraryIr`], but wraps every generated statement in one
/// CFG-valid advisory instruction group with a typed captured input when the
/// entry block has a parameter.
#[derive(Debug)]
pub struct ArbitraryIrAdvisoryGroup {
    pub blocks: IRBlocks<()>,
    pub types: TypeTable,
    pub inputs: Vec<IrValue>,
}

impl<'a> Arbitrary<'a> for ArbitraryIrAdvisoryGroup {
    fn arbitrary(u: &mut Unstructured<'a>) -> Result<Self> {
        let n_params = u.int_in_range(0usize..=4)?;
        let raw_param_types: Vec<u8> = (0..n_params)
            .map(|_| u.int_in_range(0u8..=(PRIM_TYPES.len() as u8 - 1)))
        ...
        let raw_stmts: Vec<(u8, u32, u32, u128, u128)> = (0..n_stmts)
            .map(|_| Ok((
                u8::arbitrary(u)?, u32::arbitrary(u)?, u32::arbitrary(u)?,
        ...
        let inputs = param_widths
            ...
            .collect::<Result<_>>()?;
        Ok(ArbitraryIrAdvisoryGroup { blocks, types, inputs })
    }
}

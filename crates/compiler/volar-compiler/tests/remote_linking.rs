...
use volar_compiler::{
    IrFunction, IrModule, LinkageKind, LinkageSystem, LinkedSpec, print_module_with_remotes,
    printer_ts::print_module_ts_with_imports,
};
...
fn test_remote_spec_not_inlined() {
    ...
    assert!(
        target.structs.is_empty(),
        "remote spec must not be inlined into apply() output"
    );
    assert!(
        target.traits.is_empty(),
        "remote spec traits must not be inlined"
    );
}
...
fn test_inline_spec_is_inlined() {
    ...
    assert!(
        !target.structs.is_empty(),
        "inline spec structs must be inlined"
    );
    assert!(
        !target.traits.is_empty(),
        "inline spec traits must be inlined"
    );
}
...
fn test_remote_refs_returns_type_names() {
    ...
    assert!(
        refs[0].type_names.contains(&"Bit".to_string()),
        "should include Bit"
    );
    assert!(
        refs[0].type_names.contains(&"Galois".to_string()),
        "should include Galois"
    );
    assert!(
        refs[0].type_names.contains(&"Field".to_string()),
        "should include Field trait"
    );
}
...
fn test_remote_refs_empty_for_inline_only() {
    ...
    assert!(
        ls.remote_refs().is_empty(),
        "inline-only linkage has no remote refs"
    );
}
...
fn test_ts_no_import_without_remotes() {
    ...
    assert!(
        !out.contains("from 'test-primitives'"),
        "no remotes → no test-primitives import"
    );
    assert!(
        !out.contains("from '@portal"),
        "no remotes → no portal package import"
    );
}

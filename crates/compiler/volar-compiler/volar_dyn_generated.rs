...
}).collect::<Vec<Vec<BitsInBytes>>>(), v: (0..n).map(|i| {
...
}).collect::<Vec<Vec<Bit>>>(), v: (0..n).map(|i| {
    let BitsInBytes(b) = v[i].clone();
    Bit((((b >> n) & 1) != 0))
...
}).collect::<Vec<Vec<BitsInBytes64>>>(), v: (0..n).map(|i| {
...
}).collect::<Vec<Vec<Vec<u8>>>>(), t: 0, u: 0, _phantom: PhantomData }
    }
}

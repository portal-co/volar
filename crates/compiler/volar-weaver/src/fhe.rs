...
use volar_ir::{
    boolar::{BIrBlocks, BIrStmt, LaneId},
...
pub trait FheScheme {
    ...
    /// Emit a direct one-bit oracle invocation.
    ///
    /// `oracle_name` names the oracle, `arg_exprs` are its (homomorphic)
    /// arguments, `bit` is the flattened result-bit index in
    /// declaration-result order, and `occurrence` is the stable call-site
    /// identity used by replayable evaluators.
    ///
    /// Default: panics at weave time.
    #[allow(unused_variables)]
    fn emit_oracle_bit<Q: Clone + Default>(
        &self,
        oracle_name: &str,
        arg_exprs: &[IrExpr<Q>],
        bit: usize,
        ...
        panic!(
            "FheScheme: emit_oracle_bit not implemented (oracle: '{}', bit: {}).  \
             Either implement this method or avoid circuits with oracle calls.",
            oracle_name, bit
        )
    }
    ...
    fn emit_cfg_return(&self, output_vars: &[String]) -> IrExpr {
        match output_vars {
            ...
            [single] => var(single),
            many => ir_expr(IrExprKind::Tuple(many.iter().map(|s| var(s.as_str())).collect())),
        }
    }
...
pub fn oblivious_read_loop<S: FheScheme, Q: Clone + Default>(
    ...
    for k in 0..aw {
        ...
        let cond = ir_expr_p(IrExprKind::Binary {
            op: SpecBinOp::Eq,
            left: Box::new(ir_expr_p(IrExprKind::Binary {
                op: SpecBinOp::BitAnd,
                left: Box::new(ir_expr_p(IrExprKind::Binary {
                    op: SpecBinOp::Shr,
                    left: Box::new(var(&loop_var)),
                    right: Box::new(ir_expr_p(IrExprKind::Lit(IrLit::Int(k as i128)), ctrl_prov.clone())),
                }, ctrl_prov.clone())),
                right: Box::new(ir_expr_p(IrExprKind::Lit(IrLit::Int(1)), ctrl_prov.clone())),
            }, ctrl_prov.clone())),
            right: Box::new(ir_expr_p(IrExprKind::Lit(IrLit::Int(0)), ctrl_prov.clone())),
        }, ctrl_prov.clone());
        body_stmts.push(ir_stmt_p(IrStmtKind::Let {
            pattern: IrPattern::ident(&bit_name),
            ty: None,
            init: Some(ir_expr_p(IrExprKind::If {
                cond: Box::new(cond),
                then_branch: IrBlock {
                    stmts: vec![],
    ...
    stmts.push(ir_stmt_p(IrStmtKind::Expr(ir_expr_p(IrExprKind::BoundedLoop {
        var: loop_var,
        start: Box::new(ir_expr_p(IrExprKind::Cast {
            expr: Box::new(ir_expr_p(IrExprKind::Lit(IrLit::Int(0)), ctrl_prov.clone())),
            ty: Box::new(IrType::Primitive(PrimitiveType::Usize)),
        }, ctrl_prov.clone())),
        end: Box::new(ir_expr_p(IrExprKind::Cast {
            expr: Box::new(ir_expr_p(IrExprKind::Lit(IrLit::Int(count as i128)), ctrl_prov.clone())),
            ty: Box::new(IrType::Primitive(PrimitiveType::Usize)),
        }, ctrl_prov.clone())),
        inclusive: false,
        body: IrBlock {
...
pub fn oblivious_write_loop<S: FheScheme, Q: Clone + Default>(
    ...
    body_stmts.push(ir_stmt_p(IrStmtKind::Semi(ir_expr_p(IrExprKind::Assign {
        left: Box::new(ir_expr_p(IrExprKind::Index {
            base: Box::new(var(&arr_name)),
            index: Box::new(var(&loop_var)),
        }, ctrl_prov.clone())),
        right: Box::new(scheme.emit_cmux::<Q>(
        ...
        for stmt in &block.stmts {
            match &stmt.kind {
                BIrStmt::StorageRead { storage, lane, addr } => {
                    let key = (storage.0, lane.0 as usize);
                    let entry = max_addr_len.entry(key).or_insert(0);
                    *entry = (*entry).max(addr.len());
                }
                BIrStmt::StorageWrite { storage, lane, addr, .. } => {
                    let key = (storage.0, lane.0 as usize);
                    let entry = max_addr_len.entry(key).or_insert(0);
                    *entry = (*entry).max(addr.len());
                }
...
struct FheStorageCtx {
    /// Current wire name for each cell: `(storage_id, LaneId.0, cell_index)` → wire name.
    cells: BTreeMap<(u32, usize, usize), String>,
    /// Cell count per `(storage_id, LaneId.0)` key.
...
impl FheStorageCtx {
    ...
    fn emit_read<S: FheScheme, Q: Clone + Default>(
        ...
        let count = self.counts.get(&(storage_id, lane.0 as usize)).copied().unwrap_or(0);
        if count == 0 {
        ...
        let cells: Vec<String> = (0..count)
            .map(|ci| self.cells[&(storage_id, lane.0 as usize, ci)].clone())
            .collect();
    ...
    fn emit_write<S: FheScheme, Q: Clone + Default>(
        ...
        if count == 1 {
            ...
            self.cells.insert((storage_id, lane.0 as usize, 0), new_name);
            return;
        }
        ...
        for (ci, new_name) in new_cells.into_iter().enumerate() {
            self.cells.insert((storage_id, lane.0 as usize, ci), new_name);
        }
    }
}
    ...
    S: FheScheme,
{
    ...
    for (result_id, stmt, prov) in &expanded {
        ...
        let init_expr: IrExpr<H::Output> = match stmt {
            ...
            BIrStmt::OracleBit { name, args, bit, occurrence } => {
                let arg_exprs: Vec<IrExpr<H::Output>> = args
                    .iter()
                    .map(|id| var(var_names[&id.0].as_str()))
                    .collect();
                scheme.emit_oracle_bit(name, &arg_exprs, *bit, *occurrence)
            }
            ...
            BIrStmt::StorageWrite { storage, src, lane, addr } => {
...
fn weave_fhe_cfg<S: FheScheme>(
        ...
        for (stmt_idx, ir_stmt) in ir_block.stmts.iter().enumerate() {
            ...
            match &ir_stmt.kind {
                ...
                IRStmt::StorageWrite { storage, src, ty, addr } => {
                    ...
                    stmts.push(ir_stmt_p(IrStmtKind::Semi(ir_expr(IrExprKind::Assign {
                        left: Box::new(ir_expr(IrExprKind::Index {
                            base: Box::new(var(&param_name)),
                            index: Box::new(index_expr),
                        })),
    ...
    for action_decl in &blocks.actions {
        ...
        let stub_fn = IrFunction { no_inline: false,
    ...
    if needs_bools_to_usize {
        ...
        if !already_linked {
            let helper = IrFunction { no_inline: false,
...
impl FheScheme for TfheScheme {
    ...
    fn helper_type_stubs(&self) -> Vec<IrFunction> {
        ...
        stubs.push(IrFunction { no_inline: false,
...
mod tests {
    ...
    fn build_loop_read_module(cell_count: usize, addr_width: usize) -> IrModule<IrFunction> {
        ...
        IrModule {
            ...
            type_aliases: vec![],
            consts: vec![],
            functions: vec![IrFunction { no_inline: false,

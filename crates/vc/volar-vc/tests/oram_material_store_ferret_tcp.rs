//! End-to-end durable held-material round trip over split AES and Ferret OT.
...
#[test]
fn encrypted_role_local_material_survives_a_later_held_round_over_ferret_tcp() {
    ...
    let garbler = thread::spawn(move || {
        ...
        let ot_metrics = ot.metrics();
        (output, metrics, ot_metrics)
    });
    ...
    let evaluator_ot_metrics = ot.metrics();
    assert_eq!(evaluator, vec![false]);
    let (garbler_output, garbler_metrics, garbler_ot_metrics) =
    ...
    for metrics in [garbler_ot_metrics, evaluator_ot_metrics] {
        assert!(metrics.sent_bytes > 0);
        assert!(metrics.received_bytes > 0);
    }
}

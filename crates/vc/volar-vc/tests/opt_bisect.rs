...
fn opt_bisect_field_gadgets() {
    ...
    for (name, c, inp, want) in [
        ...
        for pass in ["fold", "cse", "dce"] {
            let mut cc = want.clone();
            let _ = &mut cc; // silence
            let mut cc = match pass {
                "fold" => {
                    let mut c2 = c.clone();
                    volar_ir_opt::biir::fold_biir_blocks(&mut c2);
                    c2
                }
                "cse" => {
                    let mut c2 = c.clone();
                    volar_ir_opt::biir::cse_biir_blocks(&mut c2);
                    c2
                }
                _ => {
                    let mut c2 = c.clone();
                    volar_ir_opt::biir::dce_biir_blocks(&mut c2);
                    c2
                }
            };
            ...
            assert_eq!(
                got,
                want,
                "{pass} changed {name} semantics ({} -> {} stmts)",
                c.blocks[0].stmts.len(),
                cc.blocks[0].stmts.len()
            );
            eprintln!(
                "{name} {pass}: OK ({} -> {} stmts)",
                c.blocks[0].stmts.len(),
...
fn opt_bisect_step_circuit() {
    ...
    inp.extend(fe_in(&onef));
    inp.extend(fe_in(&zerof));
    inp.extend(fe_in(&u));
    inp.extend(fe_in(&onef));
    inp.extend(fe_in(&u));
    inp.push(false);
    ...
    for pass in ["fold", "cse", "dce"] {
        ...
        assert_eq!(
            got,
            want,
            "{pass} changed step semantics ({} -> {} stmts)",
            c.blocks[0].stmts.len(),
            cc.blocks[0].stmts.len()
        );
        eprintln!(
...
fn opt_combined_pipeline_preserves_kx_circuits() {
    ...
    // ladder step (init state, kt=1)
    {
        ...
        let zer: Fp = [0; 4];
        let mut inp = Vec::new();
        inp.extend(fe_in(&one));
        inp.extend(fe_in(&zer));
        inp.extend(fe_in(&u));
        inp.extend(fe_in(&one));
        inp.extend(fe_in(&u));
        inp.push(false);
...
fn optimized_stepped_ladder_rfc7748() {
    ...
    for i in 0..255 {
        ...
        inp.extend(fe_bits(&x2));
        inp.extend(fe_bits(&z2));
        inp.extend(fe_bits(&x3));
        inp.extend(fe_bits(&z3));
        inp.extend(fe_bits(&u));
        inp.push(swap);
        inp.push(kt);

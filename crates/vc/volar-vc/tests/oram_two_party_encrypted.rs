...
// @ai: assisted
//! S5b: the **encrypted** ORAM tree run **two-party** (garbler + evaluator over
//! OT). Combines S2 (two-party symbolic access) with S5a (the AES-encrypted
//! tree): the physical tree now stores only ciphertext, and the `tree_key` that
//! (de)encrypts it is a **garbler-held secret input** — so the evaluator, which
//! hosts the tree, sees ciphertext it cannot read.
//!
//! What this demonstrates over S2 / S5a:
//! - The encrypted begin/access circuits (with the inlined AES PRF) run as a
//!   genuine two-party garbled computation, with the tree keeping ciphertext.
//! - The `tree_key` is delivered as garbler-owned input bits (never revealed to
//!   the evaluator), so the tree's block tags/payloads stay hidden from the
//!   tree host — the obliviousness upgrade over the plaintext S2 tree.
//!
//! Scope / honesty: as in S2, the position map and stash are threaded through
//! the trusted harness as public inputs to check correctness of a short access
//! sequence (the re-based-secret threading is S4). The point here is that the
//! *tree* is now ciphertext two-party.
...
#[test]
fn s5b_two_party_encrypted_access() {
...
fn s5b_body() {
    ...
    let access_partition: Vec<InputOwner> = (0..ACCESS_I)
        .map(|i| {
            ...
            if i < MAX_STASH * eb {
                ...
                InputOwner::Garbler // tree_key (garbler-secret)
            }
        })
        .collect();
    ...
    // Obliviousness sanity: after writes, the stored tree bytes are ciphertext —
    // not the plaintext encoding of any block (the evaluator cannot read tags).
    let mut ct = Vec::new();
    ...
    assert!(
        ct.iter().any(|&b| b != 0),
        "encrypted tree stores non-trivial ciphertext"
    );
}

...
pub fn generate_split_step(
    ...
    // call, consumed when building the covering chunk's own call.
    let mut exported: BTreeMap<String, Slot> = BTreeMap::new();
    ...
    fn build_call(
        ...
        hats_or_qand_in: Option<&Slot>, // hat_k input array (qsim) or q_and_k input array (verifier)
        ...
        for p in &f.params {
            ...
            if n.starts_with("hat_") || n.starts_with("q_and_") {
                let idx: usize = n.rsplit('_').next().unwrap().parse().unwrap();
                let arr = hats_or_qand_in.expect("hat_k/q_and_k param but no input array given");
                match arr {
                    Slot::Array(base, _) => args.push(format!("{base}[{idx}].clone()")),
                    Slot::Scalar(_) => unreachable!("hats/q_and array must be Slot::Array"),
                }
                continue;
            }
            ...
            if n.starts_with("in_next_state_") {
                ...
                let arr = running_in.unwrap().2;
                if parts.len() > 1 {
                    let j: usize = parts[1].parse().unwrap();
                    args.push(format!("{arr}[{k}][{j}].clone()"));
                } else {
                    args.push(format!("{arr}[{k}].clone()"));
                }
                continue;
            }
            if n.starts_with("in_ret_val_") {
                ...
                let arr = running_in.unwrap().3;
                if parts.len() > 1 {
                    let j: usize = parts[1].parse().unwrap();
                    args.push(format!("{arr}[{m}][{j}].clone()"));
                } else {
                    args.push(format!("{arr}[{m}].clone()"));
                }
                continue;
            }
    ...
    for (i, b) in boundary.iter().enumerate() {
        ...
        let p_outcome = build_call(
            &mut out, pf, "vope_one(&delta)", entry_w, 0, None, None, None, &exported, Some((i, i + 1)), false,
        ...
        let q_outcome = build_call(
            &mut out, qf, "q_one(&delta)", entry_w, 1, Some(&p_hats), None, None, &exported, Some((i, i + 1)), false,
        ...
        let v_outcome = build_call(
            &mut out, vf, "q_one(&delta)", entry_w, 1, Some(&q_and_arr), Some(&r_ands_name), None, &exported, Some((i, i + 1)), true,
        ...
        // Only the *verifier*-side (Q) exports are threaded onward to
        // chunk functions (which are Q-typed throughout, prover/qsim's
        // own analogous exports aren't separately threaded -- QSim's own
        // export values equal the verifier's structurally but are never
        // consumed downstream, only its q_and output is).
        let _ = (p_is_active, p_done, p_next_pc, p_next_state, p_ret_vals);
        let _ = (q_is_active, q_done, q_next_pc, q_next_state, q_ret_vals);

        exported.insert(format!("is_active_{i}"), v_is_active);
        exported.insert(format!("done_{i}"), v_done);
        for (j, s) in v_next_pc.into_iter().enumerate() {
            exported.insert(format!("next_pc_{i}_{j}"), s);
        }
        for (k, s) in v_next_state.into_iter().enumerate() {
            exported.insert(format!("next_state_{i}_{k}"), s);
        }
        for (m, s) in v_ret_vals.into_iter().enumerate() {
            exported.insert(format!("ret_val_{i}_{m}"), s);
        }
    }
    ...
    let mut lo = 0usize;
    let mut running_done_acc = "_acc_init_done".to_string();
    let mut running_next_pc = "_acc_init_pc".to_string();
    let mut running_next_state = "_acc_init_state".to_string();
    let mut running_ret_vals = "_acc_init_ret".to_string();
    out.push_str(&format!("let {running_done_acc} = q_zero();\n"));
    out.push_str(&format!(
        "let {running_next_pc}: [Q<N, Galois>; {}] = core::array::from_fn(|_| q_zero());\n",
        accum_info.init.next_pc.len()
    ));
    out.push_str(&format!(
        "let {running_next_state} = ({});\n",
        (0..accum_info.init.next_state.len()).map(|_| "q_zero()".to_string()).collect::<Vec<_>>().join(", ")
    ));
    out.push_str(&format!(
        "let {running_ret_vals} = ({});\n",
        (0..accum_info.init.ret_vals.len()).map(|_| "q_zero()".to_string()).collect::<Vec<_>>().join(", ")
    ));
    ...
    for c in 0..n_chunks {
        ...
        let running_in = (running_done_acc.as_str(), running_next_pc.as_str(), running_next_state.as_str(), running_ret_vals.as_str());

        let p_outcome = build_call(
            &mut out, pf, "vope_one(&delta)", entry_w, 0, None, None, Some(running_in), &exported, Some((lo, hi)), false,
            "", "", oracle_vope.as_deref(), &format!("p_{uid}"),
        );
        let p_hats = p_outcome.finish_output.last().unwrap().clone();

        let q_outcome = build_call(
            &mut out, qf, "q_one(&delta)", entry_w, 1, Some(&p_hats), None, Some(running_in), &exported, Some((lo, hi)), false,
            "", "", oracle_q.as_deref(), &format!("q_{uid}"),
        );
        ...
        let v_outcome = build_call(
            &mut out, vf, "q_one(&delta)", entry_w, 1, Some(&q_and_arr), Some(&r_ands_name), Some(running_in), &exported, Some((lo, hi)), true,
            &all_ok_expr, &fold_state_expr, oracle_q.as_deref(), &format!("v_{uid}"),
        );
        let v_slots = v_outcome.finish_output;
        ...
        let mut idx = 0usize;
        let new_done_acc = match &v_slots[idx] { Slot::Scalar(n) => n.clone(), _ => unreachable!() }; idx += 1;
        let new_next_pc_slots: Vec<Slot> = v_slots[idx..idx + pc_w].to_vec(); idx += pc_w;
        let new_next_state_slots: Vec<Slot> = v_slots[idx..idx + st_w].to_vec(); idx += st_w;
        let new_ret_val_slots: Vec<Slot> = v_slots[idx..idx + rv_w].to_vec(); idx += rv_w;
        let v_all_ok = match &v_slots[idx] { Slot::Scalar(n) => n.clone(), _ => unreachable!() }; idx += 1;
        let v_fold_state = match &v_slots[idx] { Slot::Scalar(n) => n.clone(), _ => unreachable!() };
        ...
        let pc_arr = format!("_acc_pc_{uid}");
        out.push_str(&format!("let {pc_arr} = [{}];\n", new_next_pc_slots.iter().map(|s| match s { Slot::Scalar(n) => format!("{n}.clone()"), _ => unreachable!() }).collect::<Vec<_>>().join(", ")));
        let st_arr = format!("_acc_st_{uid}");
        out.push_str(&format!("let {st_arr} = ({});\n", new_next_state_slots.iter().map(|s| match s { Slot::Scalar(n) => format!("{n}.clone()"), _ => unreachable!() }).collect::<Vec<_>>().join(", ")));
        let rv_arr = format!("_acc_rv_{uid}");
        out.push_str(&format!("let {rv_arr} = ({});\n", new_ret_val_slots.iter().map(|s| match s { Slot::Scalar(n) => format!("{n}.clone()"), _ => unreachable!() }).collect::<Vec<_>>().join(", ")));

        running_done_acc = new_done_acc;
        running_next_pc = pc_arr;
        running_next_state = st_arr;
        running_ret_vals = rv_arr;
        lo = hi;
    }
    ...
    // ---- Finish -------------------------------------------------------
    {
        ...
        let p_outcome = build_call(
            &mut out, pf, "vope_one(&delta)", entry_w, 0, None, None, Some(running_in), &exported, None, false,
            "", "", oracle_vope.as_deref(), &format!("p_{uid}"),
        );
        let p_slots = p_outcome.finish_output;
        let p_hats = p_slots.last().unwrap().clone();
        let p_output: Vec<Slot> = p_slots[..p_slots.len() - 1].to_vec();

        let q_outcome = build_call(
            &mut out, qf, "q_one(&delta)", entry_w, 1, Some(&p_hats), None, Some(running_in), &exported, None, false,
            "", "", oracle_q.as_deref(), &format!("q_{uid}"),
        );
        let q_slots = q_outcome.finish_output;
        let q_and_arr = q_slots.last().unwrap().clone();
        let q_output: Vec<Slot> = q_slots[..q_slots.len() - 1].to_vec();
        ...
        let v_outcome = build_call(
            &mut out, vf, "q_one(&delta)", entry_w, 1, Some(&q_and_arr), Some(&r_ands_name), Some(running_in), &exported, None, true,
            &all_ok_expr, &fold_state_expr, oracle_q.as_deref(), &format!("v_{uid}"),
        );
        let v_slots = v_outcome.finish_output;

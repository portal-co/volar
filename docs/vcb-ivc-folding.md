...
| `BoundaryLink` embedding | **dual-preimage Keccak, both circuits + VOLE lowering DONE**: folding leg = `keccak_r1cs` + `KeccakDigestLink` (+ e2e bridge tests); VOLE leg = `emit_keccak256` gadget **and** `weave_keccak_check{,_verifier}` lowering it + the public-digest equality (`match`, opened/`assert_one_check`-ed) to compilable prover/verifier IR; all cross-checked vs `sha3`. **Remaining**: splice the call into the actual hybrid/storage gap boundary (wire boundary state, stream hats, open `match`) |

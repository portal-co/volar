...
|---|---|---|
| V1 | `vec_lint.rs` pass + exemption scanner + unit tests | 6 lint tests green; detects `VecType`/`VecCollect`/`VecCtorPath`; honors and fail-closes exemptions |
| V2 | `plan.rs` LUT `inputs` → inline `LutInputs`; interpreter temps → stack; `keys.rs` dual storage (`BootstrappingKeyRef`/`KeySwitchingKeyRef` borrowed + owned `Vec` + `AsBootstrappingKey`/`AsKeySwitchingKey` view traits) | spec tests green; borrowed-key test runs the pipeline with **zero heap key storage**; binfhe plan/dyn/e2e still pass |
| V3 | parse-time gate in `volar-codegen` (on by default) | binfhe vec-lint clean; a **seeded** non-exempt `Vec` file fails the gate with all three kinds; OT/FAEST pre-existing errors listed for V5 |
| V4 | weaver-output lint gate in `fhe_binfhe` tests | weaver tests green; `weave_binfhe_plan` output asserted Vec-free with no exemptions |
| V5 | rest-of-spec sweep | whole spec vec-lint clean: binfhe via genuine removal + targeted exemptions; OT/VOLE/FAEST protocol buffers via honest file-level `runtime-boundary` classification |
| V6 | AGENTS.md rule 11 + this index | landed |

### As-built notes (vs. the original plan)

- **`IrType::Vector` exists** (parses/prints as `Vec<T>`), so type-position
  detection is simpler than the planned `Struct{Custom("Vec")}`-only rule;
  both are linted. `Collect`/`CollectTyped` live on `IterTerminal` (not
  `IterStep`); the walker handles that.
- **The exemption scanner runs at parse time** in `volar-codegen` (the IR
  drops doc comments on conversion), via a `syn` walk: item attrs,
  module-block attrs, and a `//! @volar-allow-vec:` file-level form for
  codecs. The parser-side `parse_source_with_allow_vec` was *not* added
  (the workspace resolves `volar-compiler` to the git pin, not the local
  path, so compiler edits would be invisible to the bin); the collection
  lives in `volar-compiler-passes`'s bin, which is local.
- **Lowered/autogenerated code is not linted** (by design): the dyn mirror
  and generated interpreters legitimately use `Vec`; the gate is on the
  parsed spec program structure and the weaver's emitted IR only.
- **`LutSpec.entries` stayed a `Vec`** (data, not shape) with an exemption;
  `PlanOp::Lut.inputs` became the inline `LutInputs` (shape). The plan's
  interpreter arenas and eval keys kept `Vec` with documented exemptions
  (`host-interpreter`, `eval-key-store`), and keys additionally gained a
  zero-heap borrowed variant for stack-rich virtual targets.
- No new spec dependency was added (hand-rolled `LutInputs`, per the
  no-unmodeled-dependencies constraint).

...
| `MPC-EXT-STRICT-01` | Evaluator-executor strict action batch/reinsertion | Cross-process semantic and malformed-frame corpus; session/circuit/request binding | **PARTIAL:** existing strict actions are explicitly restricted to evaluator + `BothRoles`; unsupported executor/reveal policy fails closed. `StrictGateCursor` now pauses at `NeedsExternalBoundary` and resumes only after the compatibility action adapter reinserts the whole call result, separating cursor traversal from external I/O. `ExternalBatchFrame` now provides bounded, exact-length versioned manifest/reveal/clear-input/result/reinsertion envelopes that echo request identity and manifest binding; peer manifests must equal the locally derived manifest/binding, and result frames are checked against declared request/output width before reinsertion. Legacy action frames carry and verify stable request ID and reject trailing bytes, but do not transmit/authenticate the full session/circuit binding. A conservative `ExternalBatchTranscript` also validates ordered `Reveal → ClearInputs → Result → Reinserted` progression and result width before a request can complete. `EvaluatorBatchExecutor` now executes the explicit legacy evaluator + `BothRoles` profile against request-ID-bound host registrations, uses fallback without host invocation for guard false, and requires a separate reinsertion acknowledgement. Strict label transport now exposes `decode_legacy_action_reveal` (garbler exact-match decode), `offer_legacy_action_result_labels` (request-base-derived OT pairs), and cursor `action_reveal_labels` / `reinsert_action_result` helpers; the existing strict-actions TCP flow runs through these helpers. The new batch executor is not yet connected to the batch frame transport loop. No replay-resistance claim exists yet. |
